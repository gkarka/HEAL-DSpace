...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="renderHead">
        ...
        <xsl:param name="data_target" select="translate(ancestor::dri:*[dri:head]/@id,'.','_')"/>
        <xsl:variable name="head_count" select="count(ancestor::dri:*[dri:head])"/>
        
        <!--<a href="#{$data_target}" data-toggle="collapse">-->        
          <xsl:element name="h{$head_count}">
            <xsl:attribute name="data-target">#<xsl:value-of select="$data_target"/></xsl:attribute>
