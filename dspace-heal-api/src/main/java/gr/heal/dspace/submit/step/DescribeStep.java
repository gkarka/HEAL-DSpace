...
public class DescribeStep extends AbstractProcessingStep
{
            ...
            AuthorizeException
    {
        ...
        if((item.getMetadata("heal.type") != null) && (item.getMetadata("heal.type").length >0)){
            type = item.getMetadata("heal.type")[0].value;
        }
        DCInput[] inputs = null;
        try
        {
...
//                    subInfo.getSubmissionItem().isPublishedBefore());
        ...
        // HERE I HAVE TO FIND ALL INPUTS FOR THIS PAGE, NO MATTER WHAT DOCUMENT TYPES ARE!!!
        
        DCInput[] allInputs = null;
        try {
			allInputs = inputsReader.getInputs(c.getHandle()).getPageRows(
			        currentPage,
			        "ALL",
			        subInfo.getSubmissionItem().hasMultipleTitles(),
			        subInfo.getSubmissionItem().isPublishedBefore());
        ...
        for (int i = 0; i < allInputs.length; i++)
        {
            ...
            String qualifier = allInputs[i].getQualifier();
            if (qualifier == null
                    && allInputs[i].getInputType().equals("qualdrop_value"))
            {
                qualifier = Item.ANY;
            }
            item.clearMetadata(allInputs[i].getSchema(), allInputs[i].getElement(),
                    qualifier, Item.ANY);
        }
    ...
    protected void readNames(HttpServletRequest request, Item item,
            String schema, String element, String qualifier, boolean repeated, boolean isI18nable)
    {
        ...
        if (repeated)
        {
            ...
            lasts = getRepeatedParameter(request, metadataField, metadataField
                    + "_last");
            //modified by aanagnostopoulos
            langs = getRepeatedParameter(request, metadataField, metadataField+"_last_lang");
            //END aanagnostopoulos
        ...
        else
        {
            ...
            if(isI18nable && request.getParameter(metadataField+"_last_lang") != null) {
            	selectedLang = request.getParameter(metadataField+"_last_lang");
            }
            //END aanagnostopoulos
        ...
        for (int i = 0; i < lasts.size(); i++)
        {
            ...
            if ((l != null) && !((l.trim()).equals("")))
            {
                ...
                if (isAuthorityControlled)
                {
                    ...
                    {
                    	// modified by aanagnostopoulos
                        item.addMetadata(schema, element, qualifier, selectedLang,
...
//  modified by aanagnostopoulos
     ...
     *            set to true if the field is repeatable on the form
     * @param isI18nable
     *            set to true if the field is I18nable on the form
     */
    protected void readText(HttpServletRequest request, Item item, String schema,
            String element, String qualifier, boolean repeated, boolean isI18nable)
    //END aanagnostopoulos
    {
        ...
        else
        {
            ...
            //modified by aanagnostopoulos
            String selectedLang = LANGUAGE_QUALIFIER;
            if(isI18nable && request.getParameter(metadataField+"_lang") != null) {
