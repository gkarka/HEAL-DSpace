...
public class CompleteStep extends AbstractProcessingStep
{
            ...
            AuthorizeException
    {
        ...
        // commit changes to database
            if (success)
            {
                ...
                //Execute 'filter-media' script on submitted item
                String dspaceDir = ConfigurationManager.getProperty("dspace.dir");
                File binDir = new File(dspaceDir, "bin");
                File dspaceScript = new File(binDir, "dspace");
                String script = dspaceScript.getAbsolutePath()+" filter-media -i "+subInfo.getSubmissionItem().getItem().getHandle();
                log.info("**SCRIPT**: "+script);
				try {
					List<String> result;
					result = executeScript(script);
					for(String resultLine: result) {
						log.info("SCRIPT RESULT: "+resultLine);
					}
				} catch (Exception e) {
		            log.error("Caught exception while filtering media (full text extraction): ",e);
		            //swallow
				}
    ...
    //modified by aanagnostopoulos
	private List<String> executeScript(String script) throws IOException {
		...
		while ((readLine = stdIn.readLine()) != null) {
			lines.add(readLine);
		}
		in.close();
