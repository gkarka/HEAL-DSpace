...
importClass(Packages.gr.heal.dspace.app.xmlui.aspect.administrative.HealFlowItemUtils);
...
function doManageItems()
{
	...
	do {
		...
		if (cocoon.request.get("submit_find") && cocoon.request.get("identifier"))
		{
			...
			result = HealFlowItemUtils.resolveItemIdentifier(getDSContext(),identifier);
...
function doEditItemBitstreams(itemID)
{
	...
	do {
		...
		else if (cocoon.request.get("submit_delete") && cocoon.request.get("remove"))
		{
            ...
            result = HealFlowItemUtils.processReorderBitstream(getDSContext(), itemID, cocoon.request);
        }
	} while (true)
}
...
function doEditItemMetadata(itemID, templateCollectionID)
{
	...
	do {
		...
		else if (cocoon.request.get("submit_add"))
		{
			...
			result = HealFlowItemUtils.processAddMetadata(getDSContext(),itemID,cocoon.request);
		}
		else if (cocoon.request.get("submit_update"))
		{
			// Update the item
			result = HealFlowItemUtils.processEditItem(getDSContext(),itemID,cocoon.request);
		}
	} while (true)
}
...
function doCurateItem(itemID, task) 
{
    ...
    do {
           if (cocoon.request.get("select_curate_group"))
	   {
           ...
           {
                result = HealFlowItemUtils.processCurateItem(getDSContext(), itemID, cocoon.request);
           }
           else if (cocoon.request.get("submit_queue_task"))
           {
                result = HealFlowItemUtils.processQueueItem(getDSContext(), itemID, cocoon.request);
           }
    } while (true);
}
...
function doDeleteItem(itemID)
{
	...
	if (cocoon.request.get("submit_confirm"))
	{
		...
		assertAuthorized(Constants.ITEM, itemID, Constants.DELETE);
		var result = HealFlowItemUtils.processDeleteItem(getDSContext(),itemID);
...
function doWithdrawItem(itemID)
{
	...
	if (cocoon.request.get("submit_confirm"))
	{
		...
		// authorization check performed directly by the dspace-api
		// assertAdministrator();
		var result = HealFlowItemUtils.processWithdrawItem(getDSContext(),itemID);
		return result;
	}
	return null;
}
...
function doReinstateItem(itemID)
{
	...
	if (cocoon.request.get("submit_confirm"))
	{
		...
		var result = HealFlowItemUtils.processReinstateItem(getDSContext(),itemID);
		return result;
	}
	return null;
}
...
function doMoveItem(itemID)
{
    ...
    do {
        ...
        else if (cocoon.request.get("submit_move"))
        {
            ...
            result = HealFlowItemUtils.processMoveItem(getDSContext(), itemID, collectionID, inherit);
        }
    } while (result == null || !result.getContinue());
...
function doAddBitstream(itemID)
{
    ...
    do {
        ...
        {
            // Upload the file
            result = HealFlowItemUtils.processAddBitstream(getDSContext(),itemID,cocoon.request);
        }
    } while (result == null || ! result.getContinue())
...
function doEditBitstream(itemID, bitstreamID)
{
    ...
    do {
        ...
        else if (cocoon.request.get("submit_save"))
        {
            ...
            result = HealFlowItemUtils.processEditBitstream(getDSContext(),itemID,bitstreamID,bitstreamName,primary,description,formatID,userFormat);
        }
    } while (result == null || ! result.getContinue())
...
function doDeleteBitstreams(itemID, bitstreamIDs)
{
    ...
    if (cocoon.request.get("submit_confirm"))
    {
        ...
        var result = HealFlowItemUtils.processDeleteBitstreams(getDSContext(),itemID,bitstreamIDs);
        return result;
    }
    return null;
