...
public class UploadStep extends AbstractSubmissionStep
{
    ...
    protected static final Message T_column7 = 
            message("xmlui.Submission.submit.UploadStep.column7");
    ...
    protected static final Message T_hidden = 
            message("xmlui.Submission.submit.UploadStep.hidden_bitstream");
    protected static final Message T_hidden_help = 
            message("xmlui.Submission.submit.UploadStep.hidden_bitstream_help");
    protected static final Message T_unknown = 
            message("xmlui.Submission.submit.ReviewStep.unknown");
    protected static final Message T_hidden_yes = 
            message("xmlui.Submission.submit.UploadStep.hidden_bitstream_yes");
    protected static final Message T_hidden_no = 
            message("xmlui.Submission.submit.UploadStep.hidden_bitstream_no");
            ...
            UIException, SQLException, IOException, AuthorizeException
    {
		...
		//modified by aanagnostopoulos
		//Get a list of all files in the hidden bundle
		Bundle[] hiddenBundles = item.getBundles("HIDDEN");
		Bitstream[] hiddenBitstreams = new Bitstream[0];
		if (hiddenBundles.length > 0)
		{
			hiddenBitstreams = hiddenBundles[0].getBitstreams();
		}
		//merge the lists
		bitstreams = (Bitstream[]) ArrayUtils.addAll(bitstreams, hiddenBitstreams);
		//END aanagnostopoulos
    	...
    	if (!workflow)
    	{
	        ...
	        //modified by aanagnostopoulos
	        Radio uploadHidden = upload.addItem().addRadio("hidden");
	        uploadHidden.setLabel(T_hidden);
	        uploadHidden.setHelp(T_hidden_help);
	        uploadHidden.addOption("VISIBLE", T_hidden_no);
	        uploadHidden.addOption("HIDDEN", T_hidden_yes);
	        uploadHidden.setOptionSelected("VISIBLE");
	        //END aanagnostopoulos
        ...
        if (bitstreams.length > 0 || workflow)
		{
	        ...
	        // modified by aanagnostopoulos
	        header.addCellContent(T_column7); // hidden bitstream (ie belongs to 'HIDDEN' bundle)
	        // END aanagnostopoulos
	        ...
	        for (Bitstream bitstream : bitstreams)
	        {
	        	...
	        	//modified by aanagnostopoulos
	        	//get bundle where bitstream belongs to
	        	...
	        	if(bundle.getName().equalsIgnoreCase("ORIGINAL")) {
	        		...
	        		//primary bitstream selection should not be applicable for other bundles
	        		row.addCellContent("");
	        	}
	        	//END aanagnostopoulos
	            ...
	            //modified by aanagnostopoulos
	            if (bundle.getName().equalsIgnoreCase("HIDDEN"))
	            {
	            	row.addCellContent(T_hidden_yes);
	            }
	            else
	            {
	            	row.addCellContent(T_hidden_no);
	            }
	            //END aanagnostopoulos
